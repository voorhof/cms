<x-cms-layout>
    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="row">
                <div class="col-md-6">
                    <form method="POST" action="{{ route(config('cms.route_name_prefix').'.roles.store') }}" class="needs-validation" novalidate>
                        @csrf
                            ...
                            @forelse($permissions as $permission)
                                <div class="form-check mb-1">
                                    <input class="form-check-input {{ $errors->has('permissions.*') ? 'is-invalid' : '' }}"
                                           ...
                                           name="permissions[]" id="permission{{ $loop->index }}" {{ is_array(old('permissions')) && in_array($permission, old('permissions')) ? 'checked' : '' }}>
                        ...
                        <div class="d-flex flex-wrap gap-3 align-items-center justify-content-between py-1">
                            ...
                            <a href="{{ route(config('cms.route_name_prefix').'.roles.index') }}" class="btn btn-dark">
